		...
		var Form = A.Component.create({
			ATTRS: {
				...
				strings: {
					value: {
						...
						requestErrorMessage: Liferay.Language.get(
							'there-was-an-error-when-trying-to-validate-your-form'
						),
						requiredFields: Liferay.Language.get(
							'all-fields-marked-with-x-are-required'
						)
					}
				},
			...
			prototype: {
				...
				submit: function() {
					...
					instance.validate(function(hasErrors) {
						if (!hasErrors) {
							var formNode = instance.getFormNode();

							var currentPageInput = formNode.one(
								'#' +
				...
				_afterFormRender: function() {
					...
					if (formNode) {
						var ddmFormBuilderAppNode = formNode.one(
							'.ddm-form-builder-app'
						);

						if (ddmFormBuilderAppNode) {
							ddmFormBuilderAppNode.removeClass(
